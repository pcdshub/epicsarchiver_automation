#!/usr/bin/env python
'''
This script gets a list of PVS that are disconnected and then pauses those that
have not connected for a specified amount of time
'''
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', "--timeout", default="5",
                        help="Specify the timeout to wait for all the PV's to connect")
    parser.add_argument('-k', "--skiplivenesscheck", action="store_true",
                        help="Specify the liveness check for pausing PV's")
    parser.add_argument("url", help="This is the URL to the mgmt bpl interface "
                        "of the appliance cluster. For example, "
                        "http://arch.slac.stanford.edu/mgmt/bpl")
    parser.add_argument("disconnTimeout", help="Pause those PV's that have not "
                        "connected for this amount of time (in minutes")

    args = parser.parse_args()
    configureLogging(args.verbose)

    if not args.url.endswith('bpl'):
        logger.error("The URL %s needs to point to the mgmt bpl; for example, "
                     "http://arch.slac.stanford.edu/mgmt/bpl", args.url)
        sys.exit(1)
    ...
    pvsThatHaveNotConnectedForTimeout = list(filter(lambda x : (
        abs((datetime.datetime.fromtimestamp(float(x['noConnectionAsOfEpochSecs']))
             - datetime.datetime.now()).total_seconds()) > int(args.disconnTimeout)*60
    ), pvList))

    if not pvsThatHaveNotConnectedForTimeout:
        logger.info("There are no PV's that have been disconnected for more than %s minute(s)",
                    args.disconnTimeout)
        sys.exit(0)

    logger.info("%s PVs have been disconnected for more than %s minutes",
                len(pvsThatHaveNotConnectedForTimeout), args.disconnTimeout)
    pvNames = [x['pvName'] for x in pvsThatHaveNotConnectedForTimeout]

    def breakIntoBatches(ll, n):
