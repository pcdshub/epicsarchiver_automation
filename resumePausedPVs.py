#!/usr/bin/env python
'''
This script gets a list of PVS that are paused and then resumes the archive
request for each active PV.  Optionally, we can pass in a folder with a list of
.archive files.  Only those paused PV's which are present in archive files that
have been modified recently will be checked.
'''
...
def getPVsFromRecentlyChangedArchiveFiles(rootFolder, filenamepattern, ignoreolder):
    ...
    for changedFile in changedFiles:
        ...
        pvConfigEntries = [shlex.split(x) for x in
...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', "--rootFolder",
                        help="The root folder of all the IOC archive request files")
    parser.add_argument('-p', "--filenamepattern",
                        help="The extended shell matching pattern used to "
                        "determine archive request files, for example,"
                        " */archive/*.archive",
    ...
    if args.rootFolder and args.filenamepattern:
        ...
        recentlyChangedPVs = getPVsFromRecentlyChangedArchiveFiles(
            args.rootFolder, args.filenamepattern, args.ignoreolder
        )
    ...
    def breakIntoBatches(llist, batch_size):
        '''
        Check for liveness and resume in batches specified by the batch size
        '''
        for i in range(0, len(llist), batch_size):
            yield llist[i:i + batch_size]
